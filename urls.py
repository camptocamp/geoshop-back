...
urlpatterns = [
    ...
    path(f'{ROOTURL}/favicon.ico', RedirectView.as_view(url=f'{settings.STATIC_URL}{ROOTURL}/favicon.ico')),
    re_path(rf'^{ROOTURL}/auth/reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', TemplateView.as_view(), name='password_reset_confirm'),
    ...
    re_path(rf'^{ROOTURL}/download/(?P<guid>\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}\b)$', views.OrderByUUIDView.as_view(), name='order_uuid'),
    re_path(rf'^{ROOTURL}/download/(?P<guid>\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}\b)/get_link$', views.DownloadLinkView.as_view(), name='order_uuid_link'),
    ...
    re_path(rf'^{ROOTURL}/auth/account-confirm-email/(?P<key>[-:\w]+)/$', TemplateView.as_view(), name='account_confirm_email'),
    ...
    re_path(rf'^{ROOTURL}/extract/orderitem/(?P<pk>[0-9]+)$', views.ExtractOrderItemView.as_view(), name='extract_orderitem'),
    ...
    re_path(rf'^{ROOTURL}/validate/orderitem/(?P<token>[a-zA-Z0-9_-]+)$', views.OrderItemByTokenView.as_view(), name='orderitem_validate'),
