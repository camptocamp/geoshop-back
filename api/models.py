...
class Document(models.Model):
    ...
    link = models.URLField(
        _("link"), help_text=_("Please complete the above URL"), max_length=2000
    )
...
class Metadata(models.Model):
    ...
    datasource = models.CharField(
        _("datasource"), max_length=260, blank=True, null=True
    )
    ...
    image_link = models.CharField(
        _("image_link"),
        max_length=250,
        default=settings.DEFAULT_METADATA_IMAGE_URL,
        blank=True,
    )
    ...
    data_last_update_date = models.DateField(
        _("data_last_update_date"), null=True, blank=True
    )
    ...
    def image_tag(self):
        if self.image_link is None or self.image_link == "":
            return mark_safe(
                '<img src="%s%s" />' % (settings.MEDIA_URL, "no_image.jpg")
            )
...
class MetadataContact(models.Model):
    ...
    contact_person = models.ForeignKey(
        Identity,
        models.CASCADE,
        verbose_name=_("contact_person"),
        limit_choices_to={"is_public": True},
    )
    metadata_role = models.CharField(_("role"), max_length=150, default=_("Manager"))
...
class Pricing(models.Model):
    ...
    class PricingType(models.TextChoices):
        ...
        FROM_CHILDREN_OF_GROUP = "FROM_CHILDREN_OF_GROUP", _(
            "From children products of this group"
        )
    ...
    def get_price(self, polygon):
        ...
        """
        price = ProductPriceCalculator.get_price(pricing_instance=self, polygon=polygon)
...
class Product(models.Model):
    ...
    class ProductStatus(models.TextChoices):
        ...
        PUBLISHED_ONLY_IN_GROUP = "PUBLISHED_ONLY_IN_GROUP", _(
            "Published only in group"
        )
    ...
    provider = models.ForeignKey(
        ...
        limit_choices_to={"groups__name": "extract"},
    )
    ...
    def thumbnail_tag(self):
        ...
        return mark_safe(
            '<img src="%s%s" />' % (settings.MEDIA_URL, self.thumbnail_link)
        )
...
class Order(models.Model):
    ...
    invoice_reference = models.CharField(
        _("invoice_reference"), max_length=255, blank=True
    )
    email_deliver = models.EmailField(
        _("email_deliver"), max_length=254, blank=True, null=True
    )
    ...
    def set_price(self):
        ...
        if items == []:
            return False
        self.total_without_vat = Money(0, settings.DEFAULT_CURRENCY)
        self.processing_fee = Money(0, settings.DEFAULT_CURRENCY)
        for item in items:
    ...
    def next_status_on_extract_input(self):
        ...
        if (
            OrderItem.OrderItemStatus.IN_EXTRACT in items_statuses
...
class OrderItem(models.Model):
    ...
    def available_formats(self):
        queryset = ProductFormat.objects.filter(product=self.product).values_list(
            "data_format__name", flat=True
        )
    ...
    def _get_price_values(self, price_value):
        if self.price_status == OrderItem.PricingStatus.PENDING:
            LOGGER.info(
                "You are trying to get a pricing value but pricing status is still PENDING"
            )
            return None
        return price_value
    ...
    def set_price(self, price=None, base_fee=None):
        ...
        if self.order.order_type.name == "Utilisateur permanent":
            if self.product.free_when_subscribed:
                if self.order.client.identity.subscribed or (
                    self.order.invoice_contact is not None
                    and self.order.invoice_contact.subscribed
                ):
                    self._price = Money(0, settings.DEFAULT_CURRENCY)
                    self._base_fee = Money(0, settings.DEFAULT_CURRENCY)
        ...
        if self.order.order_type.name in (
            "Communal",
            "Cantonal",
            "Fédéral",
            "Académique",
        ...
        if self.product.pricing.pricing_type != Pricing.PricingType.MANUAL:
            if (
                ...
                == Pricing.PricingType.FROM_CHILDREN_OF_GROUP
            ):
                ...
                self._price, self._base_fee = self.product.pricing.get_price(
                    self.order.geom
                )
...
class ProductFormat(models.Model):
    product = models.ForeignKey(
        ...
        models.CASCADE,
        verbose_name=_("product"),
        related_name="product_formats",
    )
    data_format = models.ForeignKey(
        DataFormat, models.CASCADE, verbose_name=_("data_format")
    )
